  ...
  models:
    ...
    - model: |
      ...
      questions: 
        ...
        - "Take the derivative of the probability function for slotted ALOHA and for unslotted ALOHA with respect to <code>G</code>, and set it equal to 0 to maximize the function.  Let <code>x</code> be a constant 1 for a single transmit node.  At what value of <code>G</code> is each maximized?  Plug in this value and set <code>x</code> equal to 1 to observe the maximum transmission efficiency.  Hint: use the product rule!"        
    - model: |
